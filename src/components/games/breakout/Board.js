...
export default function Board() {
  ...
  // keep ref in sync so the animation loop sees latest value
  useEffect(() => {
  ...
  useEffect(() => {
    const render = () => {
      ...
      if (player.lives === 0 && !gameOverRef.current) {
        // Show a proper modal instead of alert
        setGameOver(true);
      }
  ...
  return (
    <div style={{ textAlign: "center" }}>
      ...
      >
        <p>You lost all your lives. Want to try again?</p>
      </Modal>
    </div>
  );
}